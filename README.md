...
## Should Similar Projects Be Grouped?

Yes. This repository will be much easier to understand if similar notebooks are grouped into folders. The current flat layout makes strong research work look like loose experiments. A cleaner structure would be:

```text
ML_DL/
  README.md
  crat_adversarial_robustness/
    README.md
    CRAT_Adversarial_Attack!.ipynb
    crat-adversarial-attack_V2.0.ipynb
    crat_adversarial_attack_V2_0.ipynb
    RRS_Final.ipynb
  literature_survey/
    README.md
    LiteratureSurvey_Model__Experiment.ipynb
  potato_disease_classification/
    README.md
    AI Model(Computer_Vision_Model).ipynb
    Custom CNN.ipynb
    caiac-efficientnet-b0.ipynb
    resnet-50.ipynb
    new_experiment_custom_light_weight_CNN_XAI_Three-way_Clustering.ipynb
  trusting_cnn_predictions_three_way_analysis/
    README.md
    icdm-*.ipynb
  vision_model_evidence_under_agricultural_domain_shift/
    README.md
    icmla-stage-1-small-cnns.ipynb
    aciids-resnet.ipynb
    aciids-stage-2b-vit.ipynb
  learning_examples/
    README.md
    Feed_Forward_Algorithm.ipynb
    Feed_Forward_Algorithm_(FFNN).ipynb
```

Recommended next step: keep this repo as a research archive, but add group folders and short group-level README files. For maximum Google/GitHub visibility, the most important published works can also become separate polished repositories later, especially:

- `crat-adversarial-robustness`
- `cnn-literature-survey-analysis`
- `trusting-cnn-predictions-three-way-analysis`
- `vision-model-evidence-agricultural-domain-shift`

## Notes

- Several notebooks contain older Colab links that still reference the previous GitHub username `MH-1294`. The notebooks still work as code archives, but those badges/links should be updated if you want every notebook page to reflect the new `dmmehedi` username.
- Some notebooks are alternate versions of the same experiment. Keeping the latest version and moving older versions into an `archive/` folder would make the repository cleaner.
